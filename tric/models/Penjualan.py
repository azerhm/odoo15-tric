...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='tric.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for rec in self:
            a = sum(self.env['tric.detailpenjualan'].search([('penjualan_id', '=', rec.id)]).mapped('subtotal'))
            rec.total_bayar = a
    ...
    #         a=[]
    #         for rec in self:
    #             a = self.env['tric.detailpenjualan'].search([('penjualan_id','=',rec.id)])
    #             print(a)
    #         for ob in a:
    ...
    def write(self, vals):
        ...
        for rec in self:
            b = self.env['tric.detailpenjualan'].search([('penjualan_id', '=', rec.id)])
            print(a)
            print(b)
            for databaru in b:
                if databaru in a:
...
class DetailPenjualan(models.Model):
    _name = 'tric.detailpenjualan'
    _description = 'New Description'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(comodel_name='tric.penjualan', string='Detail Penjualan')
    barang_id = fields.Many2one(comodel_name='tric.barang', string='List Barang')
    ...
    def create(self, vals):
        ...
        if record.qty:
            self.env['tric.barang'].search([('id', '=', record.barang_id.id)]).write(
