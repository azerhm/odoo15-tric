...
class penjualanreport(models.TransientModel):
    ...
    def action_penjualan_report(self):
        ...
        print(filter)
        penjualan = self.env['tric.penjualan'].search_read(filter)
        print(penjualan)
        data = {
            'form': self.read()[0],
        ...
        print(data)
        return self.env.ref('tric.wizzard_penjualanreport_pdf').report_action(self, data=data)